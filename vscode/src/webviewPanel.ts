...
export class MemoryGraphPanel {
    ...
    public updateGraph(graphPath: string, format: 'svg' | 'png', programOutput?: { stdout: string; stderr: string }): void {
        ...
        if (format === 'svg') {
            ...
            this._panel.webview.html = this._getGraphHtml(svgContent, 'svg', programOutput);
        } else {
            // For PNG, convert to base64
            const pngBuffer = fs.readFileSync(graphPath);
            const base64 = pngBuffer.toString('base64');
            this._panel.webview.html = this._getGraphHtml(base64, 'png', programOutput);
        }
    }
...
<body>
    ...
    private _getGraphHtml(content: string, format: 'svg' | 'png', programOutput?: { stdout: string; stderr: string }): string {
